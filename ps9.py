...
car_class_method = "Drive Forward"
...
car_method_description = "when pedal is pushen down drive forward"
...
flickr_response = flickrREST(cache_fname = "flickr.response.txt", extra_params = {'tags':"sunset", 'per_page': 50 })
...
x = open("flickr_data.txt","r")
result = x.read()
flickr_diction = json.loads(result[14:-1])

# For testing your data extraction. Uncomment the following to run the test.
test.testEqual(flickr_diction,json.loads(open("flickr_data.txt").read()[14:-1]))
...
photo_ids_list=[]
for dictionary in flickr_diction['photos']['photo']:
    photo_ids_list.append(dictionary['id'])
...
tags_dic = {}
for each in photo_ids_list:
    x = flickrREST (method = 'flickr.photos.getInfo', extra_params = {'photo_id':each})
    x = json.loads(x[14:-1])
    for thing in x['photo']['tags']['tag']:
        if thing ['_content'] in tags_dic:
            tags_dic[thing['_content']]+=1
        else:
            tags_dic[thing['_content']]= 1
print tags_dic
...
sorted_flickr= sorted(tags_dic.keys(), key = lambda x:tags_dic[x], reverse = True)
