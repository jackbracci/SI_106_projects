...
sorted_fall_list = sorted(fall_list, key = lambda x: len(x), reverse=True)
...
for a in s.readlines():
    a.strip()
    if 'z' in a and len(a) == 4: #used legnth of 4 instead of 3
        c.append(a.strip())
short_z_words = sorted(c, reverse = True)
...
for line in f.readlines():
    ...
    if 'z' in line:
        d[line] = score(line)
best_z_words = sorted(d.keys(), reverse = True, key = lambda x: d[x])[:10]
